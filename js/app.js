    ...
    function menuInit() {
        if (document.querySelector(".icon-menu")) document.addEventListener("click", (function(e) {
            if (bodyLockStatus && e.target.closest(".icon-menu")) {
                const isBodyLocked = document.documentElement.classList.contains("lock");
                const isMenuOpen = document.documentElement.classList.contains("menu-open");
                if (isBodyLocked && isMenuOpen) functions_menuClose();
                if (!isBodyLocked && !isMenuOpen) menuOpen();
                console.log("+");
            }
        }));
    }
 ...
 */    var MEDIA_PREFERS_REDUCED_MOTION = "(prefers-reduced-motion: reduce)";
    ...
    const mainSliderElement = document.querySelector(".product__main-slider");
    const thumbnailSliderElement = document.querySelector(".product__thumbnail-slider");
    if (mainSliderElement) {
        var mainSlider = new Splide(mainSliderElement, {
            type: "fade",
            pagination: false,
            arrows: false
        }).mount();
    ...
    da.init();
    if (document.querySelector("[data-filter-btn]")) document.addEventListener("click", (function(e) {
        if (bodyLockStatus && e.target.closest("[data-filter-btn]")) {
            bodyLockToggle();
            document.documentElement.classList.toggle("filter-open");
            functions_menuClose();
        }
    }));
    document.querySelectorAll("[data-phone]").forEach((function(phoneInput) {
        let tmp = "+38 0";
        phoneInput.value = tmp;
        phoneInput.addEventListener("input", (function(e) {
            let formattedValue = "+38 0";
            let replaceValue = " ";
            this.value;
            let value = this.value.replace(/\D/g, "");
            if (value.length <= 3) tmp = formattedValue;
            if (value.length == 4) tmp = formattedValue + value[3];
            if (value.length == 5) tmp = formattedValue + value[3] + value[4];
            if (value.length == 6) tmp = formattedValue + value[3] + value[4] + replaceValue + value[5];
            if (value.length == 7) tmp = formattedValue + value[3] + value[4] + replaceValue + value[5] + value[6];
            if (value.length == 8) tmp = formattedValue + value[3] + value[4] + replaceValue + value[5] + value[6] + value[7];
            if (value.length == 9) tmp = formattedValue + value[3] + value[4] + replaceValue + value[5] + value[6] + value[7] + replaceValue + value[8];
            if (value.length == 10) tmp = formattedValue + value[3] + value[4] + replaceValue + value[5] + value[6] + value[7] + replaceValue + value[8] + value[9];
            if (value.length == 11) tmp = formattedValue + value[3] + value[4] + replaceValue + value[5] + value[6] + value[7] + replaceValue + value[8] + value[9] + replaceValue + value[10];
            if (value.length >= 12) tmp = formattedValue + value[3] + value[4] + replaceValue + value[5] + value[6] + value[7] + replaceValue + value[8] + value[9] + replaceValue + value[10] + value[11];
            this.value = tmp;
            tmp = "";
        }));
    }));
